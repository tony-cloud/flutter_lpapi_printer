...
class LpRasterCommandBuilder {
  ...
  Future<LpRasterPage> rasterize(
    ...
    final printableWidth = math.max(width, options.printableWidthPx ?? width);
    final alignOffset = switch (options.alignment) {
      LpPrintAlignment.left => 0,
      LpPrintAlignment.center => (printableWidth - width) ~/ 2,
      LpPrintAlignment.right => printableWidth - width,
    };
    final targetWidth =
        printableWidth + options.marginLeftPx + options.marginRightPx;
    final targetHeight = height + options.marginTopPx + options.marginBottomPx;
    ...
    for (var y = 0; y < height; y += 1) {
      final targetY = y + options.marginTopPx;
      ...
      for (var x = 0; x < width; x += 1) {
        ...
        if (!black) continue;
        final targetX = x + options.marginLeftPx + alignOffset;
  ...
  Uint8List _lineBytesCommand(int bytesPerRow) {
    return LpPacket.commandBytes(
      0x27,
      LpPacket.variableLength(math.max(0, bytesPerRow)),
    );
  }
  ...
  Uint8List _lineCountCommand(int lineCount) {
    return LpPacket.commandBytes(
      0x26,
      LpPacket.variableLength(math.max(0, lineCount)),
    );
  }
  ...
  List<Uint8List> _rowCommands(LpRasterPage page) {
    ...
    for (var row = 0; row < page.height; row += 1) {
      ...
      final rowBytes = page.bytes.sublist(
        rowStart + first,
        rowStart + last + 1,
      );
  ...
  ({int maxRowDots, int maxRollingDots}) _heatDensity(
    LpRasterPage page,
    int dpi,
  ) {
    final bytesPerRow = (page.width + 7) ~/ 8;
